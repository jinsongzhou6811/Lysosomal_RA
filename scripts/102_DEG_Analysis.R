...
for (dataset in datasets) {
  ...
  ggsave(file.path(deg_dir, paste0(dataset, "_volcano.pdf")), volcano_plot, width = 8, height = 6)
...
if (nrow(deg_summary) > 0) {
  ...
  if (length(gene_sets) >= 2) {
    venn_file_pdf <- file.path(summary_dir, "DEGs_Venn_Diagram.pdf")
    ...
    if (length(common_genes) > 0) {
      ...
      pdf(venn_file_pdf, width = 8, height = 10)
