...
plot_kME_heatmap <- function(cor_mat, title, file_path, threshold = R_THRESHOLD) {
  ...
  w_in <- max(6, ncol(cor_mat) * 0.6 + 2)
  h_in <- max(5, nrow(cor_mat) * 0.5 + 2)
  
  # Derive stem from file_path (strip .pdf extension if present)
  stem <- sub("\\.pdf$", "", file_path)
  ...
  # [MODIFIED] Triple-format output
  pdf(paste0(stem, ".pdf"), width = w_in, height = h_in)
  eval(heatmap_expr); dev.off()
  ...
  log_msg(paste("Heatmap saved:", basename(stem), "(.pdf/_300.png/_600.tiff)"))
}
