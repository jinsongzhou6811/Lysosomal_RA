...
# Outputs: .pdf (vector), _300.png (submission), _300.tiff (300dpi RGB LZW 8-bit)
# ========================================================
save_fig <- function(p, stem, w_in, h_in) {
  ...
  # 300 dpi TIFF: RGB (type="cairo"), LZW compression, 8-bit
  tiff(paste0(stem, "_300.tiff"), width = w_in, height = h_in,
...
wlog("03_volcano saved (.pdf/_300.png/_300.tiff)")
...
if (nrow(tgt_res) >= 2) {
  ...
  tryCatch({
    ...
    do.call(pheatmap, hm_args); dev.off()
    tiff(paste0(hm_stem, "_300.tiff"), width = 8, height = hm_h, units = "in", res = 300, compression = "lzw", type = "cairo")
    do.call(pheatmap, hm_args); dev.off()
    wlog("04_heatmap_targets saved (", nrow(tgt_mat), " genes, .pdf/_300.png/_300.tiff)")
  }, error = function(e)
