...
run_lysosomal_analysis <- function(target_ds) {
  ...
  for (type in c("undirected", "directed")) {
    ...
    pdf(file.path(main_output_dir, paste0("lysosome_network_", type, ".pdf")))
  ...
  for (ds in datasets) {
    ...
    if (file.exists(go_file)) {
      ...
      if (!is.null(p)) {
        ggsave(file.path(main_output_dir, paste0("gsea_go_bubble_plot_", ds, ".pdf")),
               p, width = 10, height = 10)   # ← 修改尺寸
        log_msg("GSEA GO", paste(ds, "bubble plot saved"))
      }
    }
    ...
    if (file.exists(reactome_file)) {
      ...
      if (!is.null(p)) {
        ggsave(file.path(main_output_dir, paste0("gsea_reactome_bubble_plot_", ds, ".pdf")),
               p, width = 8, height = 7)
        log_msg("GSEA Reactome", paste(ds, "bubble plot saved"))
      }
    }
    ...
    if (length(kegg_files) > 0) {
      ...
      if (!is.null(kegg_df) && nrow(kegg_df) > 0) {
        ...
        if (!is.null(p)) {
          ggsave(file.path(main_output_dir, paste0("gsea_kegg_bubble_plot_", ds, ".pdf")),
                 p, width = 8, height = 7)
          log_msg("GSEA KEGG", paste(ds, "bubble plot saved"))
        }
      } else {
  ...
  report_plot <- ggplot(nes_data, aes(x = ID, y = NES)) +
    ...
    theme_minimal()
  ggsave(file.path(main_output_dir, "validation_report.pdf"), report_plot)
