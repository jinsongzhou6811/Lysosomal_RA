...
ggsave(file.path(output_base_dir, "lysosome_deg_overlap.pdf"), p, width = 8, height = 6)
log_message("Comprehensive lysosomal DEG overlap bar chart saved", log_file)
...
for (dataset in datasets) {
  ...
  ggsave(file.path(output_base_dir, dataset, "lysosome_deg_overlap.pdf"), p, width = 6, height = 4)
  log_message(sprintf("%s: Dataset-specific bar chart saved", dataset), log_file)
}
...
if (nrow(final_results) > 0) {
  ...
  ggsave(file.path(output_base_dir, "lysosome_degs_intersection.pdf"), p, width = 8, height = 6)
  log_message(sprintf("Intersection gene count bar chart saved to: %s", file.path(output_base_dir, "lysosome_degs_intersection.pdf")), log_file)
}
...
for (geo_id in datasets) {
  ...
  # Save as PDF
  output_file <- file.path(volcano_dir, paste0("volcano_lysosome_", geo_id, ".pdf"))
  tryCatch({
    ggsave(output_file, p_volcano, width = 9.5, height = 7.5, dpi = 300)
    log_message(paste("✓ Volcano plot saved:", basename(output_file)), log_file)
  }, error = function(e) {
