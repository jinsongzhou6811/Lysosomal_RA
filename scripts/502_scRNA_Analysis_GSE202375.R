...
module_name <- "05_scRNA_Analysis\\GSE202375" # ←←← Modify here!!
# ------------------------------------------------------
...
# Define file path
rds_file <- file.path(data_dir, "GSE202375_Synovial_tissue_T_cells_normalized_expression.rds")
if (file.exists(rds_file)) {
  cat("Local RDS file already exists:", rds_file, "\n")
} else {
  stop("GSE202375 RDS file not found, please ensure the file is located in the data directory")
}
  ...
  list(
    ...
    load_func = function() {
      ...
      if (inherits(data, "Seurat")) {
        obj <- UpdateSeuratObject(data)
        cat("Upgraded to Seurat v5 object via UpdateSeuratObject\n")
        ...
        obj <- CreateSeuratObject(counts = data, project = "GSE202375")
      } else {
      ...
      obj@meta.data$sample <- "GSE202375"
      return(obj)
    }
  )
)
...
preprocess_dataset <- function(obj, dataset_name, base_output_dir, qc_params) {
  ...
  counts_data <- LayerData(obj, assay = "RNA", layer = "counts")
  data_data   <- LayerData(obj, assay = "RNA", layer = "data")
  ...
  if (all(dim(data_data) == 0)) {
    ...
    LayerData(obj, layer = "data") <- LayerData(obj, layer = "counts")
    cat("data layer empty after normalization, using counts layer\n")
  }
  ...
  if (!"cell_type" %in% colnames(obj@meta.data)) {
    ...
    tryCatch({
      ...
      expr_matrix     <- as.matrix(LayerData(obj, assay = "RNA", layer = "data"))
...
analyze_genes <- function(obj, dataset_name, output_dir, target_genes, reference_gene = NULL) {
  ...
  tryCatch({
    available_genes <- target_genes[target_genes %in% rownames(LayerData(obj, assay = "RNA", layer = "data"))]
    if (!is.null(reference_gene) && reference_gene %in% rownames(LayerData(obj, assay = "RNA", layer = "data"))) {
    ...
    # ---- Expression matrix ----
    expr_matrix <- as.matrix(LayerData(obj, assay = "RNA", layer = "data")[available_genes, , drop = FALSE])
    ...
    if ("cell_type" %in% colnames(obj@meta.data)) {
      tryCatch({
        ...
        obj_temp <- obj
        data_layer <- LayerData(obj_temp, layer = "data")
        data_layer[available_genes, ] <- log10(expr_matrix_temp)
        LayerData(obj_temp, layer = "data") <- data_layer
    ...
    if (!is.null(reference_gene) && reference_gene %in% rownames(LayerData(obj, assay = "RNA", layer = "data"))) {
      ...
      valid_genes <- c(reference_gene, target_genes)[c(reference_gene, target_genes) %in%
                                                       rownames(LayerData(obj, assay = "RNA", layer = "data"))]
      if (length(valid_genes) > 0) {
        expr_ref <- as.matrix(LayerData(obj, assay = "RNA", layer = "data")[valid_genes, , drop = FALSE])
        ...
        if ("cell_type" %in% colnames(obj@meta.data)) {
          ...
          sparsity_by_celltype <- lapply(cell_types, function(ct) {
            ...
            if (length(cells) > 10) {
              expr_ct <- as.matrix(LayerData(obj, assay = "RNA", layer = "data")[valid_genes, cells, drop = FALSE])
        ...
        ref_positive_cells <- colnames(obj)[LayerData(obj, assay = "RNA", layer = "data")[reference_gene, ] > 0]
        if (length(ref_positive_cells) > 10) {
          expr_ref_pos <- as.matrix(LayerData(obj, assay = "RNA", layer = "data")[valid_genes, ref_positive_cells, drop = FALSE])
      ...
      tryCatch({
        ...
        for (b in seq(1, num_genes, batch_size)) {
          ...
          for (gene in batch_genes) {
            expr <- LayerData(obj, layer = "data")[gene, ]
    ...
    tryCatch({
      ...
      if (length(gene_entrez) == 0) {
        cat("No ENTREZ IDs found, skipping GO/KEGG analysis\n")
      } else {
    ...
    tryCatch({
      ...
      cds <- new_cell_data_set(
        expression_data = LayerData(obj, assay = "RNA", layer = "counts"),
