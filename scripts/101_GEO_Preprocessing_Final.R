...
for (geo_id in geo_list) {
  ...
  if (geo_id == "GSE55235") {
    ...
    groups <- c(rep("Control", 10), rep("RA", 13), rep("OA", 10))
  } else if (geo_id == "GSE55584") {
...
pre_boxplot_stem <- file.path(base_dir, "plots", paste0("pre_normalization_boxplot_", log_timestamp))
save_fig(boxplot_pre, pre_boxplot_stem, w_in = 12, h_in = 6)
write_log(sprintf("Saved pre-normalization boxplot to %s (.pdf/_300.png/_600.tiff)", pre_boxplot_stem))
...
boxplot_post <- ggplot(expr_melt_post, aes(x=Var2, y=value)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
# [MODIFIED] Triple-format output via save_fig
post_boxplot_stem <- file.path(base_dir, "plots", paste0("post_normalization_boxplot_", log_timestamp))
save_fig(boxplot_post, post_boxplot_stem, w_in = 12, h_in = 6)
write_log(sprintf("Saved post-normalization boxplot to %s (.pdf/_300.png/_600.tiff)", post_boxplot_stem))
...
# [MODIFIED] Triple-format output via save_fig (15cm x 10cm ≈ 5.91in x 3.94in)
pca_pre_stem <- file.path(base_dir, "plots", paste0("geo_analysis_pca_pre_batch_", log_timestamp))
save_fig(pca_plot_obj_pre, pca_pre_stem, w_in = 5.91, h_in = 3.94)
write_log(sprintf("Saved pre-batch PCA plot to %s (.pdf/_300.png/_600.tiff)", pca_pre_stem))
...
pca_plot_obj <- ggplot(pca_data, aes(x=PC1, y=PC2, color=Group, shape=Batch)) +
  ...
  theme(legend.position="right", text = element_text(size=10))
# [MODIFIED] Triple-format output via save_fig (15cm x 10cm ≈ 5.91in x 3.94in)
pca_post_stem <- file.path(base_dir, "plots", paste0("geo_analysis_pca_post_batch_", log_timestamp))
save_fig(pca_plot_obj, pca_post_stem, w_in = 5.91, h_in = 3.94)
write_log(sprintf("Saved post-batch PCA plot to %s (.pdf/_300.png/_600.tiff)", pca_post_stem))
# ==========================================================
