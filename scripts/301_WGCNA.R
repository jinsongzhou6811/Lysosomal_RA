...
for (dataset_id in names(datasets)) {
  ...
  pdf(file.path(results_dir, "pca_pre_batch_correction.pdf"))
  ...
  dev.off()
  png(file.path(results_dir, "pca_pre_batch_correction.png"), width = 800, height = 600)
  plot(pca_pre$x[,1], pca_pre$x[,2], col = as.numeric(phenoData$RA_status),
       main = "PCA of RMA Normalized Data", xlab = "PC1", ylab = "PC2", pch = 16)
  legend("topright", legend = levels(phenoData$RA_status),
         col = 1:length(levels(phenoData$RA_status)), pch = 16)
  dev.off()
  log_file("PCA plot before batch correction generated (PDF and PNG)", main_log)
  ...
  pdf(file.path(results_dir, "pca_post_no_batch_correction.pdf"))
  ...
  legend("topright", legend = levels(phenoData$RA_status),
         col = 1:length(levels(phenoData$RA_status)), pch = 16)
  dev.off()
  png(file.path(results_dir, "pca_post_no_batch_correction.png"), width = 800, height = 600)
  plot(pca_post$x[,1], pca_post$x[,2], col = as.numeric(phenoData$RA_status),
       main = "PCA after No Batch Correction", xlab = "PC1", ylab = "PC2", pch = 16)
  legend("topright", legend = levels(phenoData$RA_status),
         col = 1:length(levels(phenoData$RA_status)), pch = 16)
  dev.off()
  log_file("PCA plot after no batch correction generated (PDF and PNG)", main_log)
  ...
  pdf(file.path(results_dir, "module_trait_heatmap.pdf"), width = 8, height = 6)
  ...
  for (mod in significant_modules) {
    if (length(module_genes[[mod]]) >= 2) {
      ...
      if (nrow(go_enrich_df) > 0) {
        pdf(file.path(results_dir, paste0("go_enrichment_dotplot_", mod, ".pdf")), width = 10, height = 8)
      ...
      if (nrow(kegg_enrich_df) > 0) {
        pdf(file.path(results_dir, paste0("kegg_enrichment_dotplot_", mod, ".pdf")), width = 10, height = 8)
      ...
      if (nrow(reactome_enrich_df) > 0) {
        pdf(file.path(results_dir, paste0("reactome_enrichment_dotplot_", mod, ".pdf")), width = 10, height = 8)
  ...
  for (gene in target_symbols) {
    ...
    ggplot(all_info, aes(x = Category, fill = Category)) +
      geom_bar() + theme_minimal() +
      labs(title = paste("Distribution of Pathway Categories for", gene),
           x = "Category", y = "Count") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(file.path(gene_dir, paste0(gene, "_pathway_distribution.png")))
    ...
    ggplot(category_counts, aes(x = category, y = count, fill = category)) +
      ...
      labs(title = paste("Distribution of Interaction Categories for", gene),
           x = "Category", y = "Count") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(file.path(gene_dir, paste0(gene, "_interaction_distribution.png")))
  }
  ...
  if (length(significant_lysosome_genes) > 0) {
    ...
    pdf(file.path(lysosome_output_dir, "lysosome_genes_heatmap.pdf"), width = 10, height = 8)
    ...
    dev.off()
    png(file.path(lysosome_output_dir, "lysosome_genes_heatmap.png"), width = 800, height = 600)
    pheatmap(lysosome_expr_subset, scale = "column",
             annotation_row = phenoData[, "RA_status", drop = FALSE],
             main = "Lysosome Genes Heatmap in Significant Modules",
             show_rownames = TRUE, show_colnames = TRUE)
    dev.off()
    log_file("Lysosome genes heatmap generated", main_log)
  } else {
...
if (length(loaded_pres) >= 2) {
  ...
  if (nrow(Zsummary_target) > 0) {
    ...
    p_pres <- ggplot(Zsummary_target, aes(x = Module, y = Zsummary, fill = Status)) +
      ...
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(file.path(preservation_dir, "FigS2B_module_preservation.pdf"), p_pres, width = 10, height = 8)
    ggsave(file.path(preservation_dir, "FigS2B_module_preservation.png"), p_pres, width = 10, height = 8, dpi = 300)
    cat("FigS2B saved to:", preservation_dir, "\n")
  }
} else {
