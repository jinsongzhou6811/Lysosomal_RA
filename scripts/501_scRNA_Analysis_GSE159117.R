...
module_name <- "05_scRNA_Analysis\\GSE159117" # ←←← Modify here!!
# ------------------------------------------------------
...
#     numeric_version(as.character(packageVersion("Seurat"))) != "4.4.0") {
#   if (!requireNamespace("remotes", quietly = TRUE)) install.packages("remotes")
#   remotes::install_version("Seurat", version = "4.4.0", repos = "https://cran.r-project.org")
# }

library(GEOquery)
library(utils)
library(Seurat) # Must downgrade to Seurat 4.4.0
library(ggplot2)
library(dplyr)
library(patchwork)
library(hdf5r)
...
AXIS_TEXT_SIZE    <- 8     # Axis tick label size
...
dir.create("GSE159117_RAW", showWarnings = FALSE)
tar_file <- "GSE159117_RAW.tar"
h5_file  <- "GSE159117_RAW/GSM4819747_RA_filtered_feature_bc_matrix.h5"

if (file.exists(h5_file)) {
  cat("Local H5 file already exists:", h5_file, "\n")
} else {
  if (file.exists(tar_file)) {
    cat("Local TAR file already exists, decompressing...\n")
    untar(tar_file, exdir = "GSE159117_RAW")
  } else {
    cat("Local data does not exist, starting download...\n")
    gse159117_url <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE159117&format=file"
    download.file(gse159117_url, destfile = tar_file, mode = "wb")
    untar(tar_file, exdir = "GSE159117_RAW")
  }
  if (file.exists(h5_file)) {
    cat("GSE159117 H5 file preparation completed:", h5_file, "\n")
  } else {
    stop("GSE159117 H5 file not found")
  }
}
...
# Gene files
gene_files <- list(
  list(file = "Target Genes GZMB.txt",    ref = NULL),
  list(file = "Target Genes LAMP3.txt",   ref = NULL),
  list(file = "Target Genes NKG7.txt",    ref = NULL),
  list(file = "Target Genes SLC39A8.txt", ref = NULL)
)
...
datasets <- list(
  list(
    ...
    load_func = function() {
      tar_dir <- "GSE159117_RAW"
      if (!dir.exists(tar_dir)) stop("GSE159117 directory does not exist")
      h5_file <- file.path(tar_dir, "GSM4819747_RA_filtered_feature_bc_matrix.h5")
      if (!file.exists(h5_file)) stop("H5 file does not exist: ", h5_file)
      data <- Read10X_h5(h5_file, use.names = TRUE)
      if (!inherits(data, "dgCMatrix")) {
        data <- as(data, "dgCMatrix")
        cat("Converted counts to dgCMatrix sparse format\n")
      }
      obj <- CreateSeuratObject(counts = data, project = "GSE159117")
      obj@meta.data$sample <- "GSM4819747"
      return(obj)
    }
  )
)
...
preprocess_dataset <- function(obj, dataset_name, base_output_dir, qc_params) {
  ...
  counts_data <- GetAssayData(obj, assay = "RNA", slot = "counts")
  data_data   <- GetAssayData(obj, assay = "RNA", slot = "data")
  ...
  if (any(grepl("^ENSG", original_rownames))) {
    ...
    tryCatch({
      ...
      gene_map <- getBM(attributes = c("ensembl_gene_id", "external_gene_name"),
                        filters = "ensembl_gene_id", values = original_rownames, mart = ensembl)
  ...
  if (all(dim(data_data) == 0)) {
    warning("data slot empty after normalization, using counts slot")
    obj[["RNA"]]@data <- obj[["RNA"]]@counts
    cat("data slot empty after normalization, using counts slot\n")
  }
  cat("Normalization completed, data slot dimension:", dim(data_data), "\n")
  cat("data slot data distribution:", summary(as.vector(data_data)), "\n")
  ...
  if (!"cell_type" %in% colnames(obj@meta.data)) {
    cat("Using SingleR for automatic cell type annotation...\n")
    tryCatch({
      hpca_ref        <- HumanPrimaryCellAtlasData()
      expr_matrix     <- as.matrix(GetAssayData(obj, assay = "RNA", slot = "data"))
  ...
  write.csv(wilcox_results[order(wilcox_results$P_Value), ],
            file.path(results_dir, "wilcoxon_module_celltype_enrichment.csv"), row.names = FALSE)
...
analyze_genes <- function(obj, dataset_name, output_dir, target_genes, reference_gene = NULL) {
  ...
  tryCatch({
    # ---- Available genes ----
    available_genes <- target_genes[target_genes %in% rownames(GetAssayData(obj, assay = "RNA", slot = "data"))]
    if (!is.null(reference_gene) && reference_gene %in% rownames(GetAssayData(obj, assay = "RNA", slot = "data"))) {
    ...
    if (!is.null(log_file)) {
      cat(paste("Number of target genes present:", length(available_genes), "\n"),
          paste("Target genes present:", paste(available_genes, collapse = ", "), "\n"),
          file = log_file, append = TRUE)
    }
    ...
    # ---- Expression matrix ----
    expr_matrix <- as.matrix(GetAssayData(obj, assay = "RNA", slot = "data")[available_genes, , drop = FALSE])
    ...
    # ---- Correlation heatmap + expression heatmap ----
    if (length(available_genes) > 1) {
      tryCatch({
        ...
        if (length(valid_genes) < 2) {
          cat("Insufficient genes with variation, skipping correlation analysis\n")
        } else {
          ...
          suppressWarnings(pheatmap(
            expr_matrix_clean,
            filename     = file.path(output_dir, "heatmap_target_genes.png"),
            width        = 10, height = 8,
            main         = "Expression Heatmap of Target Genes",
          ...
          cat("Target gene expression heatmap saved to:", file.path(output_dir, "heatmap_target_genes.png"), "\n")
        }
      }, error = function(e) {
    ...
    # ---- DotPlot (available_genes, with pseudocount) ----
    if ("cell_type" %in% colnames(obj@meta.data)) {
      tryCatch({
        expr_matrix_temp <- expr_matrix + 1e-6
        obj_temp <- obj
        obj_temp[["RNA"]]@data[available_genes, ] <- log10(expr_matrix_temp)
        dot_plot <- DotPlot(obj_temp, features = available_genes, group.by = "cell_type") +
          theme(
        ...
        ggsave(file.path(output_dir, "dotplot_target_genes.png"),
               dot_plot, width = 12, height = 8, dpi = PLOT_DPI)
        cat("Target gene dot plot saved to:", file.path(output_dir, "dotplot_target_genes.png"), "\n")
    ...
    if (!is.null(reference_gene) && reference_gene %in% rownames(GetAssayData(obj, assay = "RNA", slot = "data"))) {
      ...
      valid_genes <- c(reference_gene, target_genes)[c(reference_gene, target_genes) %in%
                                                       rownames(GetAssayData(obj, assay = "RNA", slot = "data"))]
      if (length(valid_genes) > 0) {
        expr_ref <- as.matrix(GetAssayData(obj, assay = "RNA", slot = "data")[valid_genes, , drop = FALSE])
        ...
        if ("cell_type" %in% colnames(obj@meta.data)) {
          ...
          sparsity_by_celltype <- lapply(cell_types, function(ct) {
            ...
            if (length(cells) > 10) {
              expr_ct <- as.matrix(GetAssayData(obj, assay = "RNA", slot = "data")[valid_genes, cells, drop = FALSE])
        ...
        if ("cell_type" %in% colnames(obj@meta.data)) {
          tryCatch({
            ...
            ggsave(file.path(output_dir, paste0(tolower(reference_gene), "_dotplot.png")),
                   dot_plot, width = 12, height = 8, dpi = PLOT_DPI)
            cat("Dot plot saved to:", file.path(output_dir, paste0(tolower(reference_gene), "_dotplot.png")), "\n")
          }, error = function(e) { cat("Failed to generate dot plot:", e$message, "\n") })
        }
    ...
    # ---- Violin plots (batched) — 使用 & 确保 theme 应用到所有子图 ----
    if (length(available_genes) > 0) {
      tryCatch({
        ...
        for (b in seq(1, num_genes, batch_size)) {
          ...
          violin_plots <- VlnPlot(obj, features = batch_genes, group.by = "cell_type", pt.size = 0) &
            theme(
              axis.text.x  = element_text(size = AXIS_TEXT_SIZE, angle = AXIS_TEXT_X_ANGLE, hjust = 1, vjust = 1),
              axis.text.y  = element_text(size = AXIS_TEXT_SIZE),
              axis.title.x = element_blank(),
              text         = element_text(size = PLOT_FONTSIZE)
            )
          ggsave(file.path(output_dir, paste0("violin_plots_target_genes_batch_", ceiling(b/batch_size), ".png")),
                 violin_plots, width = 14, height = 10, dpi = PLOT_DPI)
        }
      ...
      tryCatch({
        ...
        for (b in seq(1, num_genes, batch_size)) {
          ...
          expr_batch  <- as.data.frame(t(as.matrix(GetAssayData(obj, slot = "data")[batch_genes, , drop = FALSE])))
          ...
          ggsave(file.path(output_dir, paste0("UMAP_feature_plots_batch_", ceiling(b/batch_size), ".png")),
                 feature_plots, width = 12, height = 8, dpi = PLOT_DPI)
        }
    ...
    # ---- GO/KEGG enrichment ----
    tryCatch({
      gene_entrez <- bitr(available_genes, fromType = "SYMBOL", toType = "ENTREZID",
      ...
      if (length(gene_entrez) == 0) {
        ...
        if (!is.null(ego) && nrow(ego) > 0) {
          ...
          ggsave(file.path(output_dir, "go_bubble.png"), width = 8, height = 6, dpi = PLOT_DPI)
        ...
        if (!is.null(ekg) && nrow(ekg@result) > 0) {
          ...
          ggsave(file.path(output_dir, "kegg_bubble.png"), width = 8, height = 6, dpi = PLOT_DPI)
        }
      }
    ...
    # ---- Monocle3 pseudotime ----
    tryCatch({
      gene_annotation <- data.frame(gene_short_name = rownames(obj[["RNA"]]),
                                    row.names       = rownames(obj[["RNA"]]))
      cds <- new_cell_data_set(
        expression_data = GetAssayData(obj, assay = "RNA", slot = "counts"),
      ...
      if (length(uvmr_genes) == 0) {
        ...
        cat("Actual cell types (for diagnosis):",
            paste(sort(unique(as.character(obj$cell_type))), collapse = ", "), "\n")
        cat("Selecting root cells with biological priority...\n")
        
        progenitor_keywords  <- c("HSC", "Pre-B", "Pro-B", "CMP")
        existing_progenitors <- unique(obj$cell_type)[grepl(
          paste(progenitor_keywords, collapse = "|"),
          unique(obj$cell_type), ignore.case = TRUE
        )]
        
        if (length(existing_progenitors) > 0) {
          root_cells <- WhichCells(obj, idents = existing_progenitors)
          cds <- order_cells(cds, root_cells = root_cells[1])
          cat("✓ Using immune progenitor cells as trajectory root (",
              paste(existing_progenitors, collapse = ", "), ")\n")
        } else {
          cds <- order_cells(cds)
          cat("✓ No progenitor cells found, using interactive root selection\n")
        }
        ...
        if (!is.null(p)) {
          ggsave(file.path(output_dir, "pseudotime_plot.png"),
                 p, width = 8, height = 6, dpi = PLOT_DPI)
          cat("Pseudotime trajectory plot saved to:", file.path(output_dir, "pseudotime_plot.png"), "\n")
        } else {
          cat("Pseudotime plot skipped due to error in plot_genes_in_pseudotime\n")
        }
      }
      if (!is.null(log_file)) {
        cat(paste("Pseudotime trajectory plot saved to:", file.path(output_dir, "pseudotime_plot.png"), "\n"),
            file = log_file, append = TRUE)
      }
    }, error = function(e) {
